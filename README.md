...
- **已建立控制链路审计入口**：`npm run playbook:control:audit` 会本地只读审计 registered playbooks 的步骤、schema、工具、审批、失败策略、写回目标和 governed fixture coverage，并保持 `productionReady: false`。
...
- `npm run playbook:control:audit`：本地只读 playbook 控制链路审计，检查 registered playbooks 的控制合同和 fixture coverage
