...
- **已建立控制链路审计、proposal intake、migration plan、maintenance sequence、sequence evidence、freshness/provenance、doctor、maintenance readiness 与 lifecycle handoff 入口**：`npm run playbook:control:audit` 会本地只读审计 registered playbooks 的生命周期、步骤、schema、工具、审批、失败策略、写回目标、默认 runtime guardrails 和 governed fixture coverage；`npm run playbook:lifecycle:change:check` 会检查 playbook lifecycle 变更提案；`npm run playbook:lifecycle:migration:plan:check` 会检查迁移规划是否具备 planned changes、rollback、fixture review、必需命令和 no-mutation policy；`npm run playbook:lifecycle:sequence:check` 会检查 proposal、migration plan、handoff、fixture 和 controlled-runtime 测试声明是否按顺序对齐；`npm run playbook:lifecycle:sequence:evidence:check` 会检查已记录 evidence 是否覆盖这些命令且保持 no-mutation / no-publish 边界；`npm run playbook:lifecycle:sequence:evidence:freshness:check` 会检查 evidence 是否匹配 source commit、sequence digest 和 freshness window；`npm run playbook:lifecycle:sequence:evidence:doctor` 会把 freshness/provenance findings 归类成可维护状态和下一步命令；`npm run playbook:lifecycle:maintenance:ready` 会聚合 handoff 与 evidence doctor，给出维护准入状态；`npm run playbook:lifecycle:handoff` 会聚合 control audit 与 lifecycle review，给出本地 version/deprecation handoff readiness，并保持 `productionReady: false`。
