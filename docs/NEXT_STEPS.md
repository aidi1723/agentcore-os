...
- Production Release Approval Packet: `npm run release:production-approval:check -- --approval <path>` validates green production policy evidence, reviewer identity, approval scope, expiry, rollback owner, monitoring owner, release-action decisions, risk acceptance, and approval-packet-only non-production boundaries.
...
npm run release:production-approval:check -- --approval docs/release-approvals/example-production-release-approval.json
...
npm run release:deployment:gate:check -- --gate docs/release-execution-gates/example-deployment-gate.json
npm run release:external-write:gate:check -- --gate docs/release-execution-gates/example-external-write-gate.json
npm run release:production-verification:gate:check -- --gate docs/release-execution-gates/example-production-verification-gate.json
...
- Handoff summary evidence was green, but the project still needed a single local candidate-level gate before discussing any broader delivery handoff.
- The gate needed to keep local candidate readiness separate from production release, packaging, tags, uploads, deployment, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/delivery-candidate-readiness.ts`.
- Added `scripts/delivery-candidate/check-delivery-candidate.mjs`.
- Added `npm run delivery:candidate:check`.
- Added `docs/delivery-candidates/example-local-delivery-candidate.json`.
- The command validates:
  - green handoff summary evidence;
  - green `delivery:ready:check` evidence;
  - recorded `test:controlled-runtime`, `test:core-workflows`, `lint`, `build`, and `git diff --check` evidence;
  - documentation alignment;
  - risk/deferred items and rollback notes;
  - candidate-only no-production/no-publish boundaries.
- The command emits machine-readable JSON with `deliveryClaim: "local_delivery_candidate_ready"`, `productionReady: false`, `publishingPerformed: false`, and `candidateOnly: true`.
- Added helper coverage for success, invalid handoff summary, invalid delivery readiness, invalid command evidence, boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Delivery Candidate Hardening spec](superpowers/specs/2026-07-07-delivery-candidate-hardening-design.md) and [implementation plan](superpowers/plans/2026-07-07-delivery-candidate-hardening.md).
...
- This gap is now covered through `release:production-approval:check` and `release:execution-plan:check`; the next concrete gap is individual release execution gate design, still separated from real packaging, tag creation, artifact upload, deployment, external writes, credential use, and production readiness claims.
...
- Next concrete gap: individual release execution gate design for packaging, tag creation, artifact upload, deployment, and external writes, still non-executing until separate operator decisions approve each action.

## Completed. Production Release Approval Packet

Why:

- The production release policy gate was green, but the project still needed a structured approval packet before any release execution planning could be discussed.
- The gate needed to keep approval definition separate from publishing, tag creation, packaging, artifact upload, deployment, external writes, credential use, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/production-release-approval.ts`.
- Added `scripts/release-approval/check-production-release-approval.mjs`.
- Added `npm run release:production-approval:check`.
- Added `docs/release-approvals/example-production-release-approval.json`.
- The command validates:
  - green production release policy evidence;
  - reviewer identity, approval scope, and expiry;
  - rollback owner and monitoring owner;
  - ordered command evidence for production policy, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - release action decisions for packaging, tag creation, artifact upload, deployment, and external writes;
  - risk acceptance and approval-packet-only no-production/no-publish/no-tag/no-package/no-upload/no-deploy/no-credential-use boundaries.
- The command emits machine-readable JSON with `approvalClaim: "production_release_approval_packet_defined"`, `productionReady: false`, `publishingPerformed: false`, and `approvalPacketOnly: true`.
- Added helper coverage for success, invalid production policy evidence, missing reviewer/expired approval, executed or over-authorized release actions, boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Production Release Approval Packet spec](superpowers/specs/2026-07-07-production-release-approval-packet-design.md) and [implementation plan](superpowers/plans/2026-07-07-production-release-approval-packet.md).
...
- This gate does not run the recorded commands itself.
- This gate does not publish, tag, package installers, upload artifacts, deploy, call external connectors, use credentials, or claim production readiness.
- Next concrete gap: release execution planning gates, still non-executing and still separate from real packaging, tag creation, upload, deployment, external writes, credential use, and production readiness claims.

## Completed. Release Execution Planning Gates

Why:

- The production release approval packet was green, but the project still needed a structured release execution plan before any action-family execution gate could be designed.
- The gate needed to keep planning separate from package builds, tag creation, artifact upload, deployment, external writes, credential use, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/release-execution-plan.ts`.
- Added `scripts/release-execution/check-release-execution-plan.mjs`.
- Added `npm run release:execution-plan:check`.
- Added `docs/release-execution-plans/example-release-execution-plan.json`.
- The command validates:
  - green production release approval packet evidence;
  - ordered command evidence for production approval, production policy, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - planned actions for packaging, tag creation, artifact upload, deployment, and external writes;
  - per-action owner, execution gate, command intent, rollback step, monitoring step, and blocked execution status;
  - release preconditions, rollback plan, monitoring plan, credential boundary, and planning-only no-production/no-publish/no-tag/no-package/no-upload/no-deploy/no-credential-use boundaries.
- The command emits machine-readable JSON with `executionPlanClaim: "release_execution_plan_defined"`, `productionReady: false`, `publishingPerformed: false`, and `planningOnly: true`.
- Added helper coverage for success, invalid production approval evidence, executed or approved release actions, credential/readiness breaches, execution boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Release Execution Planning Gates spec](superpowers/specs/2026-07-07-release-execution-planning-gates-design.md) and [implementation plan](superpowers/plans/2026-07-07-release-execution-planning-gates.md).
...
- This gate does not run the recorded commands itself.
- This gate does not publish, tag, package installers, upload artifacts, deploy, call external connectors, use credentials, or claim production readiness.
- Next concrete gap: individual release execution gate design for package build, tag creation, artifact upload, deployment, and external writes.

## Completed. Package Build Execution Gate

Why:

- The release execution plan was green, but the project still needed a package-build-specific gate before any package command could be considered.
- The gate needed to keep package build review separate from running `desktop:package`, creating artifacts, uploading, tagging, deployment, credential use, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/package-build-execution-gate.ts`.
- Added `scripts/release-execution/check-package-build-gate.mjs`.
- Added `npm run release:package-build:gate:check`.
- Added `docs/release-execution-gates/example-package-build-gate.json`.
- The command validates:
  - green release execution plan evidence;
  - package build request metadata for `npm run desktop:package`;
  - source/supply-chain review for license, package scripts, lockfile, dependency provenance, and tracked artifact boundary;
  - ordered command evidence for release execution plan, release hygiene, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - rollback plan, monitoring plan, artifact handling, credential boundary, package build decision, and gate-only package build boundary.
- The command emits machine-readable JSON with `packageBuildGateClaim: "package_build_execution_gate_defined"`, `productionReady: false`, `publishingPerformed: false`, and `gateOnly: true`.
- Added helper coverage for success, invalid release execution plan evidence, missing package request, over-authorized package decision, package boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Package Build Execution Gate spec](superpowers/specs/2026-07-07-package-build-execution-gate-design.md) and [implementation plan](superpowers/plans/2026-07-07-package-build-execution-gate.md).
...
- This gap is now covered by `release:tag-creation:gate:check`, `release:artifact-upload:gate:check`, `release:deployment:gate:check`, `release:external-write:gate:check`, and `release:production-verification:gate:check`; the next concrete gap is release execution approval boundary design.

## Completed. Tag Creation Execution Gate

Why:

- The package build gate was green, but the project still needed a tag-creation-specific gate before any `git tag` or tag push command could be considered.
- The gate needed to keep tag creation review separate from local tag creation, remote tag push, release creation, artifact upload, deployment, credential use, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/tag-creation-execution-gate.ts`.
- Added `scripts/release-execution/check-tag-creation-gate.mjs`.
- Added `npm run release:tag-creation:gate:check`.
- Added `docs/release-execution-gates/example-tag-creation-gate.json`.
- The command validates:
  - green package build gate evidence;
  - tag request metadata for an annotated `v1.3.0` tag;
  - tag policy review for version matching, annotation, changelog linkage, release-note linkage, and collision check;
  - source commit evidence for target commit, source branch, diff gate, and branch policy;
  - ordered command evidence for package build gate, release hygiene, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - release-note linkage, rollback plan, monitoring plan, credential boundary, tag creation decision, and gate-only tag creation boundary.
- The command emits machine-readable JSON with `tagCreationGateClaim: "tag_creation_execution_gate_defined"`, `productionReady: false`, `publishingPerformed: false`, and `gateOnly: true`.
- Added helper coverage for success, invalid package build gate evidence, missing tag request, over-authorized tag decision, tag boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Tag Creation Execution Gate spec](superpowers/specs/2026-07-07-tag-creation-execution-gate-design.md) and [implementation plan](superpowers/plans/2026-07-07-tag-creation-execution-gate.md).
...
- This gap is now covered by `release:artifact-upload:gate:check`, `release:deployment:gate:check`, `release:external-write:gate:check`, and `release:production-verification:gate:check`; the next concrete gap is release execution approval boundary design.
...
- This gap is now covered by `release:deployment:gate:check`, `release:external-write:gate:check`, and `release:production-verification:gate:check`; the next concrete gap is release execution approval boundary design.

## Completed. Deployment Execution Gate

Why:

- The artifact upload gate was green, but the project still needed a deployment-specific gate before any deployment command could be considered.
- The gate needed to keep deployment review separate from real deploy commands, external writes, store writes, credential use, and production readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/deployment-execution-gate.ts`.
- Added `scripts/release-execution/check-deployment-gate.mjs`.
- Added `npm run release:deployment:gate:check`.
- Added `docs/release-execution-gates/example-deployment-gate.json`.
- The command validates:
  - green artifact upload gate evidence;
  - deployment request metadata for the `v1.3.0` release channel target;
  - environment review for target, artifact linkage, rollback window, and maintenance window;
  - pre-deployment checks for health, config, migration impact, and smoke path while proving the gate does not execute them;
  - ordered command evidence for artifact upload gate, release hygiene, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - rollback plan, monitoring plan, credential boundary, deployment decision, and gate-only deployment boundary.
- The command emits machine-readable JSON with `deploymentGateClaim: "deployment_execution_gate_defined"`, `productionReady: false`, `publishingPerformed: false`, and `gateOnly: true`.
- Added helper coverage for success, invalid artifact upload gate evidence, blocked deployment path enforcement, pre-deployment check boundary breach, over-authorized deployment decision, command evidence drift, argument parsing, invalid JSON, and CLI result generation.
- Added [Deployment Execution Gate spec](superpowers/specs/2026-07-07-deployment-execution-gate-design.md) and [implementation plan](superpowers/plans/2026-07-07-deployment-execution-gate.md).
...
- This gap is now covered by `release:external-write:gate:check` and `release:production-verification:gate:check`; the next concrete gap is release execution approval boundary design.
...
- This gap is now covered by `release:production-verification:gate:check`; the next concrete gap is release execution approval boundary design.

## Completed. Production Verification Gate

Why:

- The external-write gate was green, but the project still needed a structured verification gate before final release execution approval could be considered.
- The gate needed to define production verification requirements without running production verification, release actions, connector calls, external writes, store writes, credential use, or production-readiness claims.

Delivered:

- Added `src/lib/executor/playbooks/production-verification-gate.ts`.
- Added `scripts/release-execution/check-production-verification-gate.mjs`.
- Added `npm run release:production-verification:gate:check`.
- Added `docs/release-execution-gates/example-production-verification-gate.json`.
- The command validates:
  - green external-write gate evidence;
  - production verification plan metadata;
  - post-action checks for smoke, artifact, deployment, external-write, and rollback validation;
  - monitoring readiness and incident/rollback readiness;
  - ordered command evidence for external-write gate, release hygiene, controlled-runtime, core workflows, lint, build, and `git diff --check`;
  - credential boundary, verification decision, and verification-only no-production/no-publish/no-credential-use boundaries.
- The command emits machine-readable JSON with `productionVerificationClaim: "production_verification_requirements_defined"`, `productionReady: false`, `publishingPerformed: false`, and `verificationOnly: true`.
- Added helper coverage for success, invalid external-write gate evidence, missing verification metadata, over-authorized verification decisions, command evidence drift, boundary breach, argument parsing, invalid JSON, and CLI result generation.
- Added [Production Verification Gate spec](superpowers/specs/2026-07-07-production-verification-gate-design.md) and [implementation plan](superpowers/plans/2026-07-07-production-verification-gate.md).
...
- Next concrete gap: release execution approval boundary design.
...
## Recommended Next. Release Execution Approval Boundary

Suggested scope:

- Use `npm run release:production-verification:gate:check -- --gate docs/release-execution-gates/example-production-verification-gate.json` as the current production verification baseline before drafting final execution approval boundaries.
