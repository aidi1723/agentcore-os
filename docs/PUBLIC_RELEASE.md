...
1. `v1.1.0` as the current stable public distribution baseline
2. earlier tags as historical context, packaging traceability, and release evolution record

This means the public repository should now treat `v1.1.0` as the recommended external starting point for evaluation, installation, and release-note reference. Earlier stable and beta tags remain useful for history, but should no longer be presented as the primary public recommendation.

In practical terms:

- `v1.1.0` is the current public stable snapshot to recommend to new external users
...
- public docs, installation guidance, and GitHub release copy should align around `v1.1.0`
...
- If the document is intended to function as an externally referenced release note, a versioned release-note filename such as `docs/releases/v1.1.0.md` may be easier to index from `README.md` and GitHub Releases.
