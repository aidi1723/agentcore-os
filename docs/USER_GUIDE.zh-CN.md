...
如果你要进一步验证桌面壳、本地 sidecar 和运行时链路，可继续执行：
...
5. 点击 `测试连接`
6. 回到主界面，先运行一个核心应用
7. 如果你要验证桌面壳或 sidecar，再进入 `设置 -> 引擎核心` 检查运行时状态
...
在 `设置 -> 大模型与助手` 中，当前版本新增了“专家角色白名单”。

它的作用不是让系统无限加角色，而是只保留少量高价值、可验证、可审计的数字员工能力。
...
`Reality Checker` 负责在关键输出后指出事实跳跃、逻辑漏洞、过度承诺和需要人工确认的点。

这意味着，系统默认仍然遵循“生成 -> 复核 -> 人审 -> 入库”的稳定路径，而不是直接外发。
...
1. 在 `Deal Desk` 录入一条询盘
2. 运行资格判断
3. 让系统生成首轮跟进草稿
4. 在 `Email Assistant` 完成人工审核
5. 在 `Personal CRM` 完成本轮流程
6. 把销售资产草稿确认入 `Knowledge Vault`
7. 在 `Knowledge Vault` 里直接复用到下一条销售线索

### 客服链路

1. 在 `Support Copilot` 录入一条工单
2. 生成建议回复
3. 查看 `Reality Checker` 的复核意见
4. 把 FAQ / 升级边界草稿确认入 `Knowledge Vault`
5. 在 `Knowledge Vault` 中复用到新的客服问题

### 知识资产层

当前版本的 `Knowledge Vault -> 流程资产` 已支持：
