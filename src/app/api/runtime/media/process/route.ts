...
function ensureDataUrlFromBase64(maybeBase64: string) {
  ...
  // default to png; remote media runtimes can return jpg too, but this is a safe start
...
function uploadsRoot() {
  return path.join("/tmp", "agentcore-os", "uploads");
}

function isAllowedLocalMediaInput(fileUrl: string) {
  if (!path.isAbsolute(fileUrl)) return false;
...
function validateMediaInput(params: { fileUrl: string; engineUrl: string }) {
  if (parseHttpUrl(params.fileUrl)) {
    return params.engineUrl ? null : "远程 fileUrl 需要配置运行时引擎处理";
  }
  if (!params.engineUrl && !isAllowedLocalMediaInput(params.fileUrl)) {
    return "本地视频处理只允许读取通过 AgentCore 上传目录保存的文件";
  }
  return null;
}
...
async function localVideoFramesExecute(params: {
  ...
  if (clipName) {
    videoSrc = `/api/runtime/media/assets/${encodeURIComponent(clipName)}`;
  }
...
async function callRemoteMediaRuntime(params: {
  engineUrl: string;
  token: string;
  prompt: string;
  fileUrl: string;
      ...
      headers: {
        ...
        ...(params.token
          ? {
              ...
              "X-AgentCore-Media-Token": params.token,
            }
          : {}),
      },
      body: JSON.stringify({
        model: "agentcore-media-runtime",
        stream: false,
        temperature: 0.2,
        messages: [
          {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const headerEngineUrl =
      req.headers.get("x-agentcore-media-runtime-url") ??
      req.headers.get("x-openclaw-engine-url");
    const headerToken =
      req.headers.get("x-agentcore-media-token") ??
    ...
    if (engineUrl && !isAllowedOutboundUrl(`${engineUrl}/v1/chat/completions`, { allowLocal: true })) {
      return NextResponse.json(
        { ok: false, error: "运行时引擎地址不在允许的连接范围内" },
    ...
    const mediaInputError = validateMediaInput({ fileUrl, engineUrl });
    if (mediaInputError) {
      return NextResponse.json(
    ...
    let text = "";
    try {
      const r = await callRemoteMediaRuntime({ engineUrl, token, prompt, fileUrl });
      res = r.res;
      text = r.text;
    } catch (err) {
