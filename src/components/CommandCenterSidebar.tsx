...
import type { CommandCenterTone, RuntimeCockpitSummary } from "@/lib/home-command-center";
    ...
    <aside className="hidden min-h-0 flex-col gap-3 overflow-y-auto pr-0.5 lg:flex">
      <section className="shrink-0 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => onOpenApp("runtime_console" as AppId)}
            className="shrink-0 rounded-md border border-slate-200 bg-slate-950 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-slate-800"
          >
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          {runtimeCockpitSummary.metrics.map((metric) => (
            ...
            >
              <div className="text-[11px] font-semibold opacity-65">{metric.label}</div>
