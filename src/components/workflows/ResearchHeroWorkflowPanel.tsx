...
import { buildResearchHeroWorkflowRecommendation } from "@/lib/hero-workflow-recommendation";
...
import {
  ...
  getResearchWorkflowScenario,
} from "@/lib/research-workflow";
import { getResearchAssetByWorkflowRunId, subscribeResearchAssets, type ResearchAssetRecord } from "@/lib/research-assets";
...
type ResearchHeroWorkflowPanelProps = {
...
export function ResearchHeroWorkflowPanel({
  workflowRunId,
  title,
  description,
  emptyHint,
  ...
  nextStep,
  actions = [],
}: ResearchHeroWorkflowPanelProps) {
  const [workflowRuns, setWorkflowRuns] = useState(getWorkflowRuns());
  const [asset, setAsset] = useState<ResearchAssetRecord | null>(() => getResearchAssetByWorkflowRunId(workflowRunId));
  const [tasks, setTasks] = useState<TaskRecord[]>(() => getTasks());
  const scenario = useMemo(() => getResearchWorkflowScenario(), []);

  useEffect(() => {
    const syncRuns = () => setWorkflowRuns(getWorkflowRuns());
    const syncAsset = () => setAsset(getResearchAssetByWorkflowRunId(workflowRunId));
    const syncTasks = () => setTasks(getTasks());
    syncRuns();
    syncAsset();
    syncTasks();
    const offRuns = subscribeWorkflowRuns(syncRuns);
    const offAssets = subscribeResearchAssets(syncAsset);
  ...
  return (
    <section className="overflow-hidden rounded-[28px] border border-amber-200 bg-[radial-gradient(circle_at_top_left,_rgba(245,158,11,0.16),_rgba(255,255,255,0.98)_38%,_rgba(239,246,255,0.94)_100%)] p-5 shadow-sm shadow-amber-100/40 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.8fr)]">
        <div className="rounded-[24px] border border-white/70 bg-white/85 p-4 backdrop-blur">
          <div className="flex flex-wrap items-center gap-2">
            <div className={["rounded-full border px-3 py-1 text-xs font-semibold", getRunClasses(run)].join(" ")}>
              {getResearchRuntimeLabel(run?.state)}
            </div>
            <div className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
              {getResearchTriggerLabel(run?.triggerType)}
            </div>
            {run?.updatedAt ? (
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
              ...
              <div className="mt-2 text-sm leading-6 text-slate-800">{source || "当前还没有研究链上下文来源。"}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Next Suggested Action</div>
              <div className="mt-2 text-sm leading-6 text-slate-800">
                {nextStep || asset?.nextAction || getResearchWorkflowNextAction(run)}
              </div>
            </div>
          </div>
                  ...
                  <div key={stage.id} className="rounded-2xl border border-slate-200 bg-white/90 p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className={["rounded-full border px-3 py-1 text-xs font-semibold", getStageClasses(state)].join(" ")}>
                        {getResearchStageStateLabel(state)}
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="rounded-[24px] border border-white/70 bg-[#0f172a] p-4 text-white shadow-lg shadow-slate-900/10">
          <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-amber-300">Local Research Asset</div>
          <div className="mt-3 text-lg font-semibold text-white">{asset?.topic || "还没有研究资产快照"}</div>
          <div className="mt-1 text-sm leading-6 text-slate-300">
            {asset?.angle || "研究摘要、洞察路由和晨报分发会逐步沉淀到这里。"}
          </div>

          <div className="mt-5 space-y-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">研究来源</div>
              <div className="mt-2 text-sm leading-6 text-slate-200">{asset?.sources || "等待研究输入写入。"} </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">最新摘要状态</div>
              <div className="mt-2 text-sm leading-6 text-slate-200">{asset?.status || "not_started"} </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">晨报 / 知识路由</div>
              <div className="mt-2 text-sm leading-6 text-slate-200">
                {asset?.latestBrief || asset?.vaultQuery || "等待研究洞察进入分发阶段。"}
              </div>
            </div>
          </div>
        </div>
